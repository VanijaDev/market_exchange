var MRC_ExchangeEscrow = artifacts.require("./MRC_Exchangeexchange_escrow.sol");
...
contract("MRC_ExchangeEscrow", (accounts) => {
    ...
    before("setup", async () => {
        exchange_escrow = await MRC_Exchangeexchange_escrow.deployed();
    ...
    it("should fail because of wrong signator", async () => {
        await asserts.throws(MRC_Userexchange_escrow.new([0, accounts[9], accounts[8]]), "each signatory address should be valid");
    });
    ...
    it("should transfer funds only after all signatories would approve", async () => {
        ...
        await exchange_escrow.sendTransaction({
            from: user,
            value: deposit
        });
        assert.equal(new BigNumber(await web3.eth.getBalance(exchange_escrow.address)).toNumber(), deposit, "wrong escrow funds after user transaction");

        //  test signatures
        let recipientBalanceBefore = new BigNumber(await web3.eth.getBalance(recipient));

        //  signatory_0
        await exchange_escrow.transferFundsTo(recipient, {
            from: signatory_0
        });
        ...
        assert.equal(new BigNumber(await web3.eth.getBalance(exchange_escrow.address)).toNumber(), deposit, "escrow funds should not be moved after signatory_0");
        ...
        assert.equal(new BigNumber(await web3.eth.getBalance(exchange_escrow.address)).toNumber(), deposit, "escrow funds should not be moved after signatory_1");
        ...
        assert.equal(new BigNumber(await web3.eth.getBalance(exchange_escrow.address)).toNumber(), 0, "escrow funds should be moved after signatory_2");
    });
    ...
    describe("Events", () => {
        it("should emit UserEscrowDeposited when deposited by user", async () => {
            ...
            let tx = await exchange_escrow.sendTransaction({
                from: user,
                value: deposit
            });
        ...
        it("should emit UserEscrowTransferred when funds moved to exchange deposit", async () => {
            ...
            //  signatory_2
            let tx_to_exchange = await exchange_escrow.transferFundsTo(recipient, {
                from: signatory_2
            });
